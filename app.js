// Quiz Antincendio - Progressive Web App v1.3.7 FIXED
// Versione con TUTTI i bug risolti e miglioramenti
...
class QuizApp {
    constructor() {
        ...
        this.highScores60s = [];
        this.badges = {};
        this.completedExams = []; // FIX v1.3.7: Track completed exams
        this.dailyGoal = {
            target: 50,
            completedToday: 0,
        ...
        // FIX v1.3.7: Cleanup on page unload
    ...
    loadPersistentData() {
        try {
            ...
            const savedExams = localStorage.getItem('completedExams'); // FIX v1.3.7
            ...
            if (savedExams) this.completedExams = JSON.parse(savedExams); // FIX v1.3.7
    ...
    savePersistentData() {
        try {
            ...
            localStorage.setItem('completedExams', JSON.stringify(this.completedExams)); // FIX v1.3.7
    ...
    // ============================================
    // FIX v1.3.7: Algoritmo shuffle ottimizzato
    shuffleArray(array) {
    ...
    selectMode(mode) {
        ...
        // FIX v1.3.7: Stop timer when selecting a new mode
        if (this.timerInterval) {
            clearInterval(this.timerInterval);
            this.timerInterval = null;
        }
        ...
        switch (mode) {
            case 'training':
                this.selectedQuestions = this.shuffleArray(this.quizData);
                this.quizState = 'quiz';
                break;
            case 'exam':
    ...
    getExamQuestions() {
        return this.shuffleArray(this.quizData).slice(0, 15);
    }
        ...
        return this.quizData
            .map(q => {
                ...
                if (lastHistory) {
                    ...
                    // FIX v1.3.7: Aumenta priorità domande nuove
                    priority = 100; // Era 50, ora 100 per avere più chance
                }
    ...
    endQuiz() {
        ...
        if (this.mode === 'timeChallenge') {
            ...
            // FIX v1.3.7: Salva esame completato correttamente
            const examResult = {
                timestamp: Date.now(),
            ...
            if (examResult.passed) {
                ...
                if (passedExams >= 10 && !this.badges.fireMaster) {
    ...
    // FIX v1.3.7: Conferma prima di uscire dal training
    confirmExitTraining() {
    ...
    reviewAnswers() {
        ...
        this.quizState = 'quiz';
        this.mode = 'review';
        // FIX v1.3.7: Copia profonda per evitare side effects
    ...
    checkDailyGoal() {
        ...
        if (this.dailyGoal.lastGoalDate !== today) {
            ...
            // FIX v1.3.7: Reset badge giornaliero
    ...
    checkBadges() {
        // FIX v1.3.7: Conta esami passati correttamente
        const passedExams = this.completedExams.filter(e => e.passed).length;
    ...
    checkHighScore60s() {
        // FIX v1.3.7: Conta le domande risposte correttamente
        const score = this.answeredQuestions.length; // Era: this.currentQuestionIndex
    ...
    resetStats() {
        if (confirm('Vuoi davvero azzerare tutte le statistiche?')) {
            ...
            this.highScores60s = [];
            this.badges = {};
            this.completedExams = []; // FIX v1.3.7
            this.dailyGoal.completedToday = 0;
            this.savePersistentData();
            this.render();
        }
            ...
            <div class="bg-white dark:bg-gray-700 rounded-xl p-6 shadow-lg mb-6">
                ...
                <div class="space-y-3">
                    ${Object.entries(currentQ.options).map(([label, text]) => {
                        ...
                        } else if (isSelected && !this.showFeedback) {
                ...
                ${userAnswer && this.showFeedback ? `
                    <div class="mt-4 p-4 rounded-lg ${userAnswer === currentQ.correct_label ? 'bg-green-100 dark:bg-green-900' : 'bg-red-100 dark:bg-red-900'}">
                        <p class="font-bold ${userAnswer === currentQ.correct_label ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'}">
                            ${userAnswer === currentQ.correct_label ? '✅ Corretto!' : '❌ Sbagliato'}
                        </p>
